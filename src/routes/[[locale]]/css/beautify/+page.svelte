<script>
  ...
  function process() {
    ...
    if (!raw) {
      error = t('cssBeautify.errEmptyInput');
      return;
    }
    error = '';
    outputText = '';
  ...
  function download() {
    ...
    const name = inputFileName ? `${inputFileName}.css` : 'output.css';
...
<div class="workspace-content">
  ...
  <section class="mb-4">
    <p class="text-sm font-medium block mb-2 m-0">{t('cssBeautify.input')}</p>
    ...
    >
      ...
      />
      <p class="text-surface-600-400 text-sm m-0">{t('cssBeautify.uploadHint')}</p>
      {#if inputFileName}
        <p class="text-sm text-primary-500 mt-1 m-0">{inputFileName}.css</p>
      {/if}
    </div>
    <p class="text-xs text-surface-500-500 mb-2">{t('cssBeautify.orPaste')}</p>
    <textarea
  ...
  <section class="flex gap-3 mb-4">
    ...
    >
      {processing ? t('common.processing') : t('cssBeautify.process')}
    </button>
    ...
    <section class="card preset-outlined-surface-200-800 overflow-hidden">
      ...
      <div class="p-4">
        ...
        <p class="text-xs text-surface-500-500 mt-2 m-0">
          {outputText.length} {t('cssBeautify.chars')} · {Math.round(new Blob([outputText]).size / 1024 * 10) / 10} KB
        </p>
      </div>
    </section>
  {/if}
